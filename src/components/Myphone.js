...
const Model = () => {
  ...
  const { scene } = useGLTF("/model1/scene2.gltf");
...
const MyScenePhone = () => {
  return (
    <Canvas
      ...
      camera={{ position: [5, 5, 5], fov: 75 }} // Ajuste do fov para um ângulo mais amplo
    >
      <ambientLight intensity={1.0} /> {/* Aumenta a intensidade da luz ambiente */}
      <pointLight position={[5, 5, 5]} intensity={1.5} /> {/* Posição e intensidade da luz */}
      <mesh
